...
setInterval(() => {
  ...
  Object.values(players).forEach(p => {
    ...
    const accelRate = p.boosting ? 4.0 : 2.0;
    ...
    Object.values(foods).forEach(f => {
      ...
      const newDx = p.x - f.x;
      const newDy = p.y - f.y;
      if (newDx * newDx + newDy * newDy < 625) {
        p.score += f.value;
  ...
  for (let i = 0; i < playerArr.length; i++) {
    ...
    for (let j = 0; j < playerArr.length; j++) {
      ...
      for (let k = 0; k < p2.segments.length; k++) {
        ...
        const dy = p1.y - seg.y;
        if (dx * dx + dy * dy < 625) {
          if (!deadPlayers.includes(p1.id)) deadPlayers.push(p1.id);
          break;
        }
      }
    }
    ...
    if (p) {
      p.segments.forEach((seg, i) => {
        if (i % 2 === 0) {
          ...
          foods[foodId] = { id: foodId, x: seg.x, y: seg.y, color: p.color, value: 3 };
          io.emit('foodSpawned', foods[foodId]);
        }
      });
      io.to(id).emit('died');
  ...
  if (eatenFoods.length > 0) io.emit('foodsEaten', eatenFoods);
  if (Object.keys(movedFoods).length > 0) io.emit('foodsMoved', movedFoods);
  if (Object.keys(updates).length > 0) io.emit('stateUpdate', updates);
